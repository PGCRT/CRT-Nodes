...
def parse_lora_configs(config_string: str):
    ...
    for i, line in enumerate(lines):
        ...
        high_lora, low_lora, high_str, low_str = parts[:4]
        enabled = True
        if len(parts) > 4:
...
def get_fallback_label(high_lora_name, high_strength, low_lora_name, low_strength):
    def format_name(name):
        if not name: return "None"
        label = os.path.basename(name)
        if label.endswith('.safetensors'): label = label[:-12]
        return label
    high_label = f"H: {format_name(high_lora_name)} @ {high_strength:.2f}"
    if low_lora_name == "LowNoise Bypassed":
        low_label = "L: Bypassed"
    else:
        low_label = f"L: {format_name(low_lora_name)} @ {low_strength:.2f}"
    return f"{high_label}\n{low_label}"
...
class WAN2_2LoraCompareSampler:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "lora_batch_config": ("STRING", { "multiline": True, "default": "none, none, 0.0, 0.0,true\n" }),
    ...
    def sample(self, model_high_noise, width, height, frame_count, model_low_noise, positive, negative, seed, lora_batch_config, steps, boundary, cfg_high_noise, cfg_low_noise, sampler_name, scheduler, sigma_shift, enable_vae_decode, create_comparison_grid, add_labels, custom_labels, label_font_size, vae=None):
        denoise = 1.0
        all_lora_configs = parse_lora_configs(lora_batch_config)
        if not all_lora_configs: raise ValueError("[CRT] No valid LoRA configurations found.")

        lora_configs = [c for c in all_lora_configs if c.get("enabled", True)]

        if not lora_configs:
            Log.fail("All LoRA configurations are disabled. Nothing to sample.")
            empty_latent = {"samples": torch.zeros([0, 4, height // 8, width // 8])}
            return (empty_latent, empty_latent, None, None, "All LoRA configurations were disabled.")

        all_custom_labels = parse_label_configs(custom_labels)
        custom_label_list = [label for i, label in enumerate(all_custom_labels) if i < len(all_lora_configs) and all_lora_configs[i].get("enabled", True)]
        ...
        if decoded_images_for_output:
            ...
            if create_comparison_grid:
                if cfg_low_noise == 0:
                    config_labels = [custom_label_list[i] if i < len(custom_label_list) and custom_label_list[i] else get_fallback_label(config["high_name"], config["high_strength"], "LowNoise Bypassed", 0.0) for i, config in enumerate(lora_configs)]
                else:
                    config_labels = [custom_label_list[i] if i < len(custom_label_list) and custom_label_list[i] else get_fallback_label(config["high_name"], config["high_strength"], config["low_name"], config["low_strength"]) for i, config in enumerate(lora_configs)]
                ...
                if add_labels:
                    ...
                    _, _, _, bottom = dummy_draw.textbbox((0,0), "Line 1\nLine 2", font=font)

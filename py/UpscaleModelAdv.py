...
class CRT_UpscaleModelAdv:
    ...
    def _manage_cache_size(cls):
        ...
        if len(cls._cache) > cls._max_cache_size:
            ...
            for key in keys_to_remove:
                colored_print(f"  🗑️ Removing old cache entry", Colors.YELLOW)
                del cls._cache[key]
    ...
    def upscale_advanced(self, image, upscale_model_name, output_multiplier, tile_count, precision, batch_size, offload_model):
        ...
        if use_tiling:
            colored_print(f"  🧩 Using tiled processing ({tile_count}x{tile_count} = {tile_count_int*tile_count_int} tiles)", Colors.YELLOW)
            colored_print(f"    📋 Each tile: ~{tile_size_px}px (no overlap)", Colors.BLUE)
        else:
            colored_print(f"  🖼️ Using single-pass processing (no tiling)", Colors.GREEN)

...
class PonyUpscaleSamplerWithInjection:
    ...
    def _upscale_image(self, image, method, model_name, target_scale):
        ...
        colored_print(f"   📐 Input: {input_w}x{input_h} → Target: {target_w}x{target_h} ({target_scale:.1f}x)", Colors.BLUE)
        
        if method == "simple_resize":
            colored_print(f"   🔄 Using simple Lanczos resize...", Colors.CYAN)
            upscaled_image = comfy.utils.lanczos(
                image.permute(0, 3, 1, 2), 
                target_w, 
                target_h
            ).permute(0, 2, 3, 1)
            colored_print(f"   ✅ Simple resize completed: {upscaled_image.shape[2]}x{upscaled_image.shape[1]}", Colors.GREEN)
            ...
            if current_h != target_h or current_w != target_w:
                colored_print(f"   🔄 Final resize to exact target using Lanczos...", Colors.CYAN)
                upscaled_image = comfy.utils.lanczos(
            ...
            colored_print(f"   ✅ Model upscaling completed: {upscaled_image.shape[2]}x{upscaled_image.shape[1]}", Colors.GREEN)
            return upscaled_image
    ...
    def _upscale_latent_simple(self, latent_samples, target_scale):
        ...
        colored_print(f"   ✅ Latent upscaling completed: {upscaled_latent.shape[2]}x{upscaled_latent.shape[3]}", Colors.GREEN)
    ...
    def _sample_tiled(self, model, positive, negative, working_latent, cfg, actual_seed, steps, sampler_name, scheduler, denoise, 
        ...
        colored_print(f"🧩 Starting tiled sampling...", Colors.HEADER)
        ...
        colored_print(f"   📋 Base tile size: {base_tile_w}x{base_tile_h} latent ({base_tile_w*8}x{base_tile_h*8} pixels)", Colors.BLUE)
        colored_print(f"   🎨 Overlap: {tile_padding:.1f}% = {overlap_w}x{overlap_h} latent ({overlap_w*8}x{overlap_h*8} pixels)", Colors.BLUE)
        ...
        for y in range(rows):
            for x in range(columns):
                ...
                colored_print(f"   🔄 Processing tile {y+1},{x+1}: {tile_w}x{tile_h} latent (seed: {tile_seed})", Colors.YELLOW)
    ...
    def execute(self, model, positive, negative, vae, cfg, seed, seed_shift, steps, sampler_name, scheduler,
        ...
        colored_print(f"   🧩 Tiling: {enable_tiling} | Grid: {tile_grid} {'(operates in latent space)' if enable_tiling == 'enable' else ''}", Colors.CYAN if enable_tiling == "enable" else Colors.YELLOW)
        ...
        if color_match_strength > 0 and original_image_for_color_match is not None:
            ...
            if original_image_for_color_match.shape[1:3] != final_image.shape[1:3]:
                colored_print(f"   🔄 Resizing reference image for color matching...", Colors.CYAN)
        ...
        if enable_upscale == "enable" and upscale_by > 1.0:
            if latent is not None and upscale_method == "simple_resize":
                ...
                colored_print(f"   🎯 Process: {original_latent_w*8}x{original_latent_h*8} → {final_w}x{final_h} ({upscale_by:.1f}x, latent space)", Colors.GREEN)
            else:
                original_w, original_h = int(final_w / upscale_by), int(final_h / upscale_by)
                colored_print(f"   🎯 Process: {original_w}x{original_h} → {final_w}x{final_h} ({upscale_by:.1f}x, {upscale_method})", Colors.GREEN)
        else:
            colored_print(f"   🎯 Process: {final_w}x{final_h} (no upscaling)", Colors.GREEN)
        
        colored_print(f"   📈 Upscaling: {'Applied' if enable_upscale == 'enable' and upscale_by > 1.0 else 'Skipped'}", Colors.GREEN)
        colored_print(f"   🧩 Tiling: {'Applied (latent space)' if enable_tiling == 'enable' else 'Skipped'}", Colors.GREEN)
        colored_print(f"   💉 Noise Injection: {'Applied' if enable_noise_injection == 'enable' else 'Skipped'}", Colors.GREEN)
        colored_print(f"   🎨 Color Matching: {'Applied' if color_match_strength > 0 and original_image_for_color_match is not None else 'Skipped'}", Colors.GREEN)

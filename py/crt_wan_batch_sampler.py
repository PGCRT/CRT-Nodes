...
class Log:
    HEADER = '\033[95m'; CYAN = '\033[96m'; GREEN = '\033[92m'; YELLOW = '\033[93m'
    RED = '\033[91m'; BLUE = '\033[94m'; ENDC = '\033[0m'; BOLD = '\033[1m'
    
    @staticmethod
    def info(msg): print(f"{Log.CYAN}[INFO] {msg}{Log.ENDC}")
    @staticmethod
    def success(msg): print(f"{Log.GREEN}[âœ“] {msg}{Log.ENDC}")
    @staticmethod
    def warn(msg): print(f"{Log.YELLOW}[âš ] {msg}{Log.ENDC}")
    @staticmethod
    def error(msg): print(f"{Log.RED}[âœ—] {msg}{Log.ENDC}")
    @staticmethod
...
class CRT_WAN_BatchSampler:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                "model_high_noise": ("MODEL", {"tooltip": "Model used for the initial high-noise denoising phase. Typically handles structure and composition."}), 
                "model_low_noise": ("MODEL", {"tooltip": "Model used for the final low-noise refinement phase. Typically handles details and fine features."}),
                "positive": ("CONDITIONING", {"tooltip": "Positive prompt conditioning. Describes what you want to generate."}), 
                "negative": ("CONDITIONING", {"tooltip": "Negative prompt conditioning. Describes what you want to avoid."}),
                "width": ("INT", {"default": 960, "min": 16, "max": 4096, "step": 16, "tooltip": "Output width in pixels. Will be quantized to nearest multiple of 16."}),
                "height": ("INT", {"default": 1536, "min": 16, "max": 4096, "step": 16, "tooltip": "Output height in pixels. Will be quantized to nearest multiple of 16."}),
                "frame_count": ("INT", {"default": 1, "min": 1, "max": 4096, "tooltip": "Number of frames to generate. Use 1 for images, >1 for videos."}),
                "batch_count": ("INT", {"default": 3, "min": 1, "max": 128, "forceInput": True, "tooltip": "Number of items to generate with different seeds. Higher values = more VRAM in parallel mode."}),
                "seed": ("INT", {"default": 4149210799938517, "min": 0, "max": 0xffffffffffffffff, "tooltip": "Starting seed for generation. Each batch item uses seed+N if increment is enabled."}),
                "increment_seed": ("BOOLEAN", {"default": True, "tooltip": "If enabled, each batch item gets seed+1, seed+2, etc. If disabled, all use the same seed."}),
                "steps": ("INT", {"default": 8, "min": 1, "max": 10000, "tooltip": "Total number of denoising steps. More steps = better quality but slower generation."}),
                "boundary": ("FLOAT", {"default": 0.875, "min": 0.0, "max": 1.0, "step": 0.001, "tooltip": "Sigma value where switching from high-noise to low-noise model occurs. Lower = earlier switch."}),
                "cfg_high_noise": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 100.0, "step": 0.1, "tooltip": "Classifier-Free Guidance scale for high-noise phase. Higher = stronger prompt adherence."}),
                "cfg_low_noise": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 100.0, "step": 0.1, "tooltip": "Classifier-Free Guidance scale for low-noise phase. Higher = stronger prompt adherence."}),
                "sampler_name": (comfy.samplers.KSampler.SAMPLERS, {"default": "res_multistep", "tooltip": "Sampling algorithm. Different samplers have different quality/speed tradeoffs."}),
                "scheduler": (comfy.samplers.KSampler.SCHEDULERS, {"default": "bong_tangent", "tooltip": "Noise schedule that controls how denoising steps are distributed."}),
                "sigma_shift": ("FLOAT", {"default": 5.0, "min": 0.0, "max": 100.0, "step": 0.01, "tooltip": "Shifts the noise schedule. Higher values = more denoising emphasis on high-frequency details."}),
                "enable_vae_decode": ("BOOLEAN", {"default": True, "tooltip": "Decode latents to images. Disable to only output latents."}),
                "enable_vae_tiled_decode": ("BOOLEAN", {"default": False, "tooltip": "Use tiled VAE decoding to reduce VRAM usage. Useful for high resolutions."}),
                "tile_size": ("INT", {"default": 512, "min": 64, "max": 4096, "step": 32, "tooltip": "Tile size for tiled VAE decode (spatial). Smaller = less VRAM but slower."}),
                "tile_overlap": ("INT", {"default": 64, "min": 0, "max": 4096, "step": 32, "tooltip": "Overlap between tiles for tiled VAE decode. Higher = fewer seams but slower."}),
                "temporal_tile_size": ("INT", {"default": 64, "min": 8, "max": 4096, "step": 4, "tooltip": "Tile size for tiled VAE decode (temporal/frames). Only relevant for video generation."}),
                "temporal_tile_overlap": ("INT", {"default": 8, "min": 4, "max": 4096, "step": 4, "tooltip": "Overlap between temporal tiles. Only relevant for video generation."}),
                "create_comparison_grid": ("BOOLEAN", {"default": True, "tooltip": "Create a side-by-side comparison grid of all batch outputs."}),
                "save_videos_images": ("BOOLEAN", {"default": True, "tooltip": "Save outputs to disk. Disable to only use outputs in workflow."}),
                "save_folder_path": ("STRING", {"default": output_dir, "tooltip": "Root folder where outputs will be saved."}),
                "save_subfolder_name": ("STRING", {"default": "FAST_BATCH", "tooltip": "Subfolder name inside the root folder for organizing outputs."}),
                "save_filename_prefix": ("STRING", {"default": "output", "tooltip": "Prefix for output filenames. Seed will be appended automatically."}),
                "fps": ("INT", {"default": 16, "min": 1, "max": 120, "tooltip": "Frames per second for video outputs. Only relevant when frame_count > 1."}),
                "processing_mode": (["Sequential", "Parallel"], {"default": "Parallel", "tooltip": "Parallel = process all batches together (fast, high VRAM). Sequential = one at a time (slow, low VRAM)."}),
                "offload_conditioning": ("BOOLEAN", {"default": False, "tooltip": "Offload text conditioning to CPU between items. Reduces VRAM slightly, forces sequential mode."}),
            }, 
            "optional": { 
                "vae": ("VAE", {"tooltip": "VAE model for encoding/decoding. Required if enable_vae_decode is True or using I2V mode."}), 
                "start_image": ("IMAGE", {"tooltip": "Starting image for Image-to-Video generation. When provided, switches to I2V mode automatically."}), 
            },
            "hidden": { 
                "prompt": "PROMPT", 
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                for i, frame in enumerate(frames): 
                    cv2.imwrite(os.path.join(temp_dir, f"frame_{i:06d}.png"), 
                ...
                ffmpeg_cmd.extend(["-c:v", "libx264", "-crf", "17", "-preset", "fast", 
                                  "-pix_fmt", "yuv420p", "-loglevel", "error", video_filepath])
    ...
    def _prepare_i2v_conditioning(self, start_image_single, vae, frame_count, width, height):
        s_image = comfy.utils.common_upscale(start_image_single[:frame_count].movedim(-1, 1), 
                                             width, height, "bilinear", "center").movedim(1, -1)
        image_padded = torch.ones((frame_count, height, width, s_image.shape[-1]), 
                                  device=s_image.device, dtype=s_image.dtype) * 0.5
        image_padded[:s_image.shape[0]] = s_image
        concat_latent_image = vae.encode(image_padded[:, :, :, :3])
        
        latent_length = ((frame_count - 1) // 4) + 1
        mask = torch.ones((1, 1, latent_length, concat_latent_image.shape[-2], 
                          concat_latent_image.shape[-1]), device=concat_latent_image.device, 
                         dtype=s_image.dtype)
    ...
    def sample(self, model_high_noise, model_low_noise, positive, negative, width, height, 
               frame_count, batch_count, seed, increment_seed, steps, boundary, cfg_high_noise, 
               cfg_low_noise, sampler_name, scheduler, sigma_shift, enable_vae_decode, 
               enable_vae_tiled_decode, tile_size, tile_overlap, temporal_tile_size, 
               temporal_tile_overlap, create_comparison_grid, save_videos_images, save_folder_path, 
               save_subfolder_name, save_filename_prefix, fps, processing_mode, offload_conditioning,
               vae=None, start_image=None, prompt=None, extra_pnginfo=None):
        ...
        if use_i2v:
            ...
            for idx, start_img in enumerate(start_images_list):
                i2v_cond = self._prepare_i2v_conditioning(start_img, vae, frame_count, 
                                                          quantized_width, quantized_height)
        ...
        base_latent = torch.zeros([1, 16, latent_temporal_depth, quantized_height // 8, 
                                   quantized_width // 8], 
        ...
        sigmas = comfy.samplers.calculate_sigmas(temp_model.get_model_object("model_sampling"), 
                                                 scheduler, steps)
        ...
        if not run_sequential:
            ...
            latents_for_handoff = comfy.sample.sample(
                mh_clone, noise, steps, cfg_high_noise, sampler_name, scheduler, 
                pos_batch, neg_batch, base_latent.repeat(batch_count, 1, 1, 1, 1), 
                start_step=0, last_step=switching_step, force_full_denoise=True, 
            ...
            if switching_step < steps:
                ...
                final_latents = comfy.sample.sample(
                    ml_clone, noise, steps, cfg_low_noise, sampler_name, scheduler, 
                    pos_batch, neg_batch, latents_for_handoff, 
        ...
        # SEQUENTIAL MODE
            ...
            for i in range(batch_count):
                ...
                pos = self._slice_conditioning(positive_offloaded, i, 
                                              device if offload_conditioning else None)
                neg = self._slice_conditioning(negative_offloaded, i, 
                                              device if offload_conditioning else None)
                ...
                latent_handoff = comfy.sample.sample(
                    mh_clone, noise, steps, cfg_high_noise, sampler_name, scheduler, 
                    pos, neg, base_latent, start_step=0, last_step=switching_step, 
                    force_full_denoise=True, disable_noise=(switching_step >= steps), 
                    callback=latent_preview.prepare_callback(mh_clone, switching_step)
                )
            ...
            if switching_step < steps:
                ...
                for i in range(batch_count):
                    ...
                    final_latent = comfy.sample.sample(
                        ml_clone, noise, steps, cfg_low_noise, sampler_name, scheduler, 
                        pos, neg, latents_for_handoff_list[i], 
                        start_step=switching_step, last_step=steps, 
                        callback=latent_preview.prepare_callback(ml_clone, steps - switching_step)
                    )
        ...
        if (enable_vae_decode or enable_vae_tiled_decode) and vae:
            ...
            for i in range(batch_count):
                ...
                if enable_vae_tiled_decode:
                    try: 
                        decoded = vae.decode_tiled(latent, tile_x=tile_size//8, tile_y=tile_size//8, 
                                                  overlap=tile_overlap//8, tile_t=temporal_tile_size, 
                ...
                if save_videos_images:
                    if frame_count == 1: 
                        self._save_image(frames.squeeze(0), save_folder_path, save_subfolder_name, 
                                        save_filename_prefix, seeds[i], prompt, extra_pnginfo)
                    else: 
                        self._save_video(frames, save_folder_path, save_subfolder_name, 
                                        save_filename_prefix, seeds[i], fps, prompt, extra_pnginfo)
        ...
        settings_str = (f"Dims: {quantized_width}x{quantized_height} | Frames: {frame_count} | "
                       f"Batches: {batch_count} | Seeds: {seeds[0]}-{seeds[-1] if increment_seed else seeds[0]}")
        
        Log.vram("Final")
        Log.success("Batch sampling complete")
        
        return ({"samples": high_noise_batch_out}, {"samples": final_latent_batch_out}, 
                images_out, grid_out, settings_str)

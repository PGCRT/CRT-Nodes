...
# --- NEW: Correct FETA (Enhance-A-Video) implementation from KJNodes ---
...
class CRT_WAN_BatchSampler:
    DESCRIPTION = """A two-phase batch sampler optimized for the WAN 2.2 model.
* Boundary: Timestep to switch from high-noise to low-noise model.
* Sigma Shift: Adjusts noise schedule for detail. 8.0 is standard for WAN.
* Low VRAM Mode: ON = slower, sequential for low memory. OFF (default) = faster, parallel using more VRAM.
* Enhance Weight: Strength of the FETA (Enhance-A-Video) effect for temporal consistency. Set to 0 to disable.
"""
    @classmethod
    def INPUT_TYPES(cls):
    ...
    def save_single_video(self, images, folder_path, subfolder_name, filename_prefix, seed, fps):
        # Unchanged
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                ffmpeg_cmd = ["ffmpeg", "-y", "-framerate", str(fps), "-i", os.path.join(temp_dir, "frame_%06d.png"), "-c:v", "libx264", "-crf", "0", "-preset", "ultrafast", "-pix_fmt", "yuv444p", video_filepath]
    ...
    def sample(self, model_high_noise, model_low_noise, positive, negative, width, height, frame_count, batch_count, seed, increment_seed, steps, boundary, cfg_high_noise, cfg_low_noise, sampler_name, scheduler, sigma_shift, enhance_weight, enable_vae_decode, create_comparison_grid, save_videos, save_folder_path, save_subfolder_name, save_filename_prefix, fps, low_vram_mode, vae=None):
        ...
        # --- THE FIX: Use latent temporal depth for the patch ---
        ...
        if enable_vae_decode and vae:
            ...
            for i in range(actual_batch_size):
                Log.info(f"Decoding video {i+1}/{actual_batch_size} (Seed: {seeds[i]})")
                ...
                if save_videos: self.save_single_video(video_frames_cpu, save_folder_path, save_subfolder_name, save_filename_prefix, seeds[i], fps)

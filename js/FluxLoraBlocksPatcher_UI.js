...
class FluxLoraBlocksPatcherUI {
    ...
    randomizeSliders(type) {
        const maxBlocks = type === 'double_blocks' ? MAX_DOUBLE_BLOCKS : MAX_SINGLE_BLOCKS;
        const defaultSum = maxBlocks * 1.0;
        ...
        let values = Array(maxBlocks).fill(0);
        let currentSum = 0;
        ...
        const scaleFactor = defaultSum / currentSum;
        values = values.map(value => {
            const scaled = value * scaleFactor;
            return Math.min(maxValue, Math.max(minValue, scaled));
        });
        
        let adjustedSum = values.reduce((sum, val) => sum + val, 0);
        let adjustment = (defaultSum - adjustedSum) / maxBlocks;
        values = values.map(val => {
            let adjusted = val + adjustment;
            return Math.min(maxValue, Math.max(minValue, adjusted));
        });
        
        adjustedSum = values.reduce((sum, val) => sum + val, 0);
        if (Math.abs(adjustedSum - defaultSum) > 1e-5) {
            const finalAdjustment = (defaultSum - adjustedSum) / maxBlocks;
            values = values.map(val => {
                let finalVal = val + finalAdjustment;
                return Math.min(maxValue, Math.max(minValue, finalVal));
            });
        }
